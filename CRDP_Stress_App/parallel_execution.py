...
# metrics collection, and the phase summary / JSON record builders.
#
######################################################################
import time
import threading
...
def protectBulkData_session(session, t_endpointCRDP, t_dataArray, t_protectionPolicy):
    ...
    from CRDP_REST_API import (
        ...
        CRDP_PROTECTED_DATA_ARRAY_NAME,
    ...
    # the entries are passed back to revealBulkData_session unchanged.
    return _loads(r)[CRDP_PROTECTED_DATA_ARRAY_NAME]


def revealBulkData_session(session, t_endpointCRDP, t_dataArray, t_protectionPolicy, t_user):
    """
...
def worker_protect_messages(task_id, indexed_messages, endpointCRDP, protectionPolicy, pbar, lock):
    ...
    Returns metrics, list of (msg_idx, protected_chunk).
    """
    ...
    try:
        for msg_idx, payloads in indexed_messages:
            ...
            c_data_array = protectBulkData_session(
...
def worker_reveal_messages(task_id, indexed_messages, endpointCRDP, protectionPolicy, r_user, pbar, lock):
    """
    ...
    try:
        for msg_idx, payloads in indexed_messages:
            ...
            r_data_array = revealBulkData_session(
                session, endpointCRDP, payloads, protectionPolicy, r_user
            )
            call_end = time.time()
...
def execute_protect_messages_parallel(messages, num_threads, endpointCRDP, protectionPolicy):
    ...
    Returns:
        AggregatedMetrics, flat c_data_array (in original payload order)
    """
    ...
    return agg_metrics, c_data_array


def execute_reveal_messages_parallel(messages, num_threads, endpointCRDP, protectionPolicy, r_user):
    """
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            for task_id, msg_list in enumerate(worker_messages):
                ...
                future = executor.submit(
                    ...
                    task_id, msg_list, endpointCRDP, protectionPolicy, r_user, pbar, progress_lock,
                )
                futures[future] = task_id
